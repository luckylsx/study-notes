...
InnoDB存储引擎最小存储单元是页，每页默认为16K，在B+树索引结构里，页可以放一行一行的数据（叶子节点），也可以放主键+指针（非叶子节点）。

以InnoDB整数字段为例，主键id为bigint类型，长度为8字节，而指针大小在InnoDB源码中设置为6字节。这样算下来就是 16384 / 14 = 1170，就是说一个页上可以存放1170个指针。N叉树的N差不多为 1200 （16K / (8b + 6b))。
...
* https://zhuanlan.zhihu.com/p/335485037